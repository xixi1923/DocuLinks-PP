...
import { getApprovedDocuments, getUserLikes, getUserFavorites, addLike, removeLike, addFavorite, removeFavorite, getDocumentById } from '@/lib/firestoreHelpers'
...
export default function ExplorePage() {
  ...
  const handleLike = async (id: string) => {
    try {
      ...
      const updatedResource = { ...resource, isLiked: !resource.isLiked }
      ...
      try {
        ...
        // Fetch updated count from Firestore
        const updatedDoc = await getDocumentById(id)
        if (updatedDoc) {
          setResources(resources.map(r => 
            r.id === id ? { ...r, isLiked: !resource.isLiked, likes: Math.max(0, updatedDoc.likes_count || 0) } : r
          ))
        }
      } catch (error) {
        // Revert on error
