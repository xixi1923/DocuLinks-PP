  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-indigo-50 flex items-center justify-center px-4 py-12 animate-gradient-x">
      <div className="w-full max-w-md animate-fade-in-up">
        {/* Decorative elements */}
        <div className="absolute top-20 right-10 w-72 h-72 bg-blue-300 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse" />
        <div className="absolute bottom-20 left-10 w-72 h-72 bg-purple-300 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse" style={{animationDelay: '1s'}} />
        
        {/* Card */}
        <div className="relative bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
          {/* Header */}
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white mb-4 shadow-lg animate-bounce">
              <LogIn size={32} />
            </div>
            <h1 className="text-3xl font-black text-slate-900 mb-2">សូមស្វាគមន៍ត្រឡប់មកវិញ</h1>
            <p className="text-slate-600">ចូលគណនីរបស់អ្នកដើម្បីបន្ត</p>
          </div>
          ...
          <form onSubmit={login} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {errors.password && <p className="text-xs text-red-600 flex items-center gap-1"><X size={12} /> {errors.password}</p>}
            </div>
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl p-4 font-black text-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-slate-500 font-medium">មិនទាន់មានគណនីទេ?</span>
            </div>
          </div>
          ...
          <Link 
            href="/auth/signup"
            className="block w-full text-center border-2 border-blue-200 text-blue-700 rounded-xl p-4 font-bold hover:bg-blue-50 hover:border-blue-300 transition-all duration-300"
          >
            បង្កើតគណនីថ្មី
          </Link>
        </div>
