...
export const getUserStats = async (userId: string) => {
  try {
    const [userDocs, comments, favorites] = await Promise.all([
    ...
    // uploads: number of documents the user created
    const uploads = userDocs.length;

    // likes: total likes across the user's uploaded documents
    const likes = userDocs.reduce((sum, doc) => sum + Math.max(0, doc.likes_count ?? 0), 0);
